        ...
        for tile in get_ring_of_radius(strategic, tank_tile, radius):
            if piece.type == "tank":
                ...
                if strategic.estimate_tile_danger(tile) & ENEMY_ARTILLERY == ENEMY_ARTILLERY:
                    ...
                    possible_tiles.append(tile)
            elif piece.type == "airplane":
                if tile in lost_territory:
                    possible_tiles.append(tile)
        else:
...
def snapshot(strategic: StrategicApi):
    global our_territory, lost_territory
    update = set()
    for x in range(strategic.get_game_width()):
        for y in range(strategic.get_game_height()):
            coordinate = common_types.Coordinates(x, y)
            danger = strategic.estimate_tile_danger(coordinate)
            if danger == OUR_TILE:
                update.add(coordinate)
    lost_territory = (c for c in our_territory if c not in update)
    our_territory = update
...
def do_turn(strategic: StrategicApi):
    ...
    for piece, command_id in attacking_pieces.items():
        ...
        if piece.type == "artillery":
            ...
            strategic.attack({StrategicPiece(piece.id, piece.type)}, tile_to_attack,
                             3 if artillery_attack[piece.id] else 1)
        elif piece.type == "antitank" or piece.type == "tank":
            strategic.attack({StrategicPiece(piece.id, piece.type)},
                             get_tile_to_attack(strategic, mass_center_of_our_territory(strategic), piece.tile, piece),
                             1)
    ...
    for builder in builders.keys():
        ...
        elif num_of_pieces_built % 20 == 18:
